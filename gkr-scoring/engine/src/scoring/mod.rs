...
pub const DOMAIN: &[u8] = b"OSUMANIA_GKR_V1";
pub const CHART_DOMAIN: &[u8] = b"OSUMANIA_GKR_CHART_V1";
...
pub fn absorb_statement(
    ...
    words.extend(g1_to_words(&st.chart.commitment));
    if st.mode == Mode::Committed {
        words.extend(g1_to_words(
            &st.trace_commitment
                .expect("mode B needs a trace commitment"),
        ));
    }
    words.extend(g1_to_words(adv));
    tr.absorb_words(&words);
}
