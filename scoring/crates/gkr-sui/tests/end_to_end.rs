use mania_gkr_sui::field::{Field, F};
use mania_gkr_sui::scoring::api::{self, Proved};
use mania_gkr_sui::scoring::session::{register_chart, verify_chart_registration};
use mania_gkr_sui::scoring::{witness, ChartRecord, Mode};
use mania_gkr_sui::testutil::{benchmark_input, random_input};
...
fn tampering_is_rejected() {
    ...
    let ok = |p: &mania_gkr_sui::scoring::ScoreProof, st: &mania_gkr_sui::scoring::Statement| {
...
fn malicious_witnesses_are_rejected() {
    use mania_gkr_sui::scoring::prover;
    use mania_gkr_sui::scoring::relation::{chart as ch, lane as ln, Kind};
    let input = fixture("demo.json");
    let record = chart_record(&input);
...
fn proof_encoding_roundtrip() {
    ...
    for mode in [Mode::Calldata, Mode::Committed] {
        ...
        let items = encode::proof_items(&proved.proof);
        ...
        bad[5] = vec![0xff; 32]; // non-canonical
