...
pub const INPUT_POLICY_V2: &[u8] = b"OSUMANIA_INPUT_POLICY_V2_KZG_BLS12381";
pub const SESSION_DOMAIN_V2: &[u8] = b"OSUMANIA_HARDWARE_SESSION_V2_BLS12381";
...
/// (48-byte compressed BLS12-381 G1).
pub fn session_digest_v2(
    ...
    s.update(trace_root);
    s.update(g1_to_bytes(trace_commitment));
    s.finalize().into()
}
...
fn chart_point(
    ...
    tr.absorb_items(&[chart_hash.to_vec(), g1_to_bytes(commitment).to_vec()]);
    tr.squeeze_n(vars)
}
