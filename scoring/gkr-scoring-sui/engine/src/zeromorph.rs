//! Zeromorph multilinear evaluation proofs over KZG/BLS12-381 with an explicit
//! degree check for the batched quotient (SPEC §7.6).
use crate::field::{
    g1_to_bytes, g2_to_bytes, Curve, Field, G1Affine, G2Affine, Group, PrimeCurveAffine,
    PrimeField, Word, F, G1, G2,
};
use crate::transcript::{keccak, Transcript};
use anyhow::{bail, ensure, Context, Result};
use halo2curves::bls12381::{multi_miller_loop, Gt};
...
impl VerifierKey {
    /// srsId = keccak256(τG2 ‖ shift[1] ‖ … ‖ shift[smax]), 96-byte compressed G2 each.
    pub fn id(&self) -> Word {
        let mut bytes = g2_to_bytes(&self.g2_tau).to_vec();
        for p in &self.g2_shift[1..=self.smax] {
            bytes.extend_from_slice(&g2_to_bytes(p));
        }
        keccak(&[&bytes])
    }
}
...
impl Srs {
    ...
    // No `.ptau` loader: snarkjs ceremonies are BN254. A production BLS12-381 SRS can be
    // taken from a public BLS12-381 powers-of-tau (e.g. the Filecoin phase-1 ceremony, 2^27
    // powers); it needs the G2 points [τ]_2 and [τ^{2^smax − 2^n}]_2 as well (see README).
    ...
    pub fn load(path: &Path) -> Result<Self> {
        ...
        let g2_shift = (0..=smax)
            ...
            .collect();
        let n = 1usize << smax;
        let raw_len = G1Affine::identity().to_raw_bytes().len();
        let mut raw = vec![0u8; n * raw_len];
...
/// Wire items: q_0..q_{n-1}, q̂, q̂', π (48-byte compressed each).
pub fn proof_items(p: &ZmProof) -> Vec<Vec<u8>> {
    p.q.iter()
        .chain([&p.qhat, &p.qhat_shift, &p.pi])
        .map(|q| g1_to_bytes(q).to_vec())
        .collect()
}
