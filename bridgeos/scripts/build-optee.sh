...
if [ "$mode" = hardware ]; then
    echo 'REFUSED: RK3566 raw OTP HUK is firmware-readable; no verified on-chip non-exportable secp256k1 signer or ROM-authenticated BL32 chain is available.' >&2
    exit 1
fi
[ "$mode" = dev ] || { echo "Usage: $0 [dev]" >&2; exit 2; }
