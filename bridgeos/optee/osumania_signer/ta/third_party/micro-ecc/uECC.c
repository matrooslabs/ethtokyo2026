...
static int uECC_sign_with_k_internal(const uint8_t *private_key,
    ...
    /* Ethereum v only carries y parity. Retry the vanishingly rare x >= n case. */
    if (uECC_vli_cmp_unsafe(p, curve->n, num_n_words) != -1) {
        return 0;
    }
    if (recovery_id) {
        *recovery_id = (uint8_t)(p[num_words] & 1u);
    }
    ...
    if (recovery_id) {
        ...
        uECC_vli_set(half_n, curve->n, num_n_words);
        uECC_vli_rshift1(half_n, num_n_words);
        if (uECC_vli_cmp_unsafe(s, half_n, num_n_words) == 1) {
...
int uECC_sign_with_k(const uint8_t *private_key,
    ...
    return uECC_sign_with_k_internal(private_key, message_hash, hash_size, k2, signature, 0, curve);
}
...
int uECC_sign(const uint8_t *private_key,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        if (uECC_sign_with_k_internal(private_key, message_hash, hash_size, k, signature, 0, curve)) {
            return 1;
        }
    }
    return 0;
...
static int uECC_sign_deterministic_internal(const uint8_t *private_key,
                            const uint8_t *message_hash,
                            unsigned hash_size,
    ...
    filled:
        ...
        if (uECC_sign_with_k_internal(private_key, message_hash, hash_size, T, signature, recovery_id, curve)) {
            return 1;
        }
...
int uECC_sign_deterministic(const uint8_t *private_key,
                            ...
                            uint8_t *signature,
                            uECC_Curve curve) {
    return uECC_sign_deterministic_internal(private_key, message_hash, hash_size,
                                            hash_context, signature, 0, curve);
}

int uECC_sign_deterministic_recoverable(const uint8_t *private_key,
                            const uint8_t *message_hash,
                            unsigned hash_size,
                            const uECC_HashContext *hash_context,
                            uint8_t *signature,
                            ...
                            uECC_Curve curve) {
    return uECC_sign_deterministic_internal(private_key, message_hash, hash_size,
                                            hash_context, signature, recovery_id, curve);
}
