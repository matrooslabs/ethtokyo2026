...
4. Both free practice and a paid run first show the same setup screen: adjust note scroll speed, visual effects, and keybinds. These changes cost no plays and do not change the paid scoring rules. Only after **Start paid run** does the client check the scorer and physical device again, preload/validate the chart, then sign `start_paid` on Sui. The resulting Session belongs to that wallet/round/chart/device. One play is spent even if capture fails; no gameplay resume.
